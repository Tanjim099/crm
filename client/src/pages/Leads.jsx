...
import { deleteLead, filterByDate, filterByProjectName, filterByStatus, getAllLeads, getLeadsByUserId, updateLeadAssign, updateLeadStatus } from '../redux/slices/leadSlice';
...
function Leads() {
    ...
    async function onFilterByDate() {
        if (filterDate !== "") {
            const res = await dispatch(filterByDate(filterDate))
            // console.log(res);
                    ...
                    <div className='flex gap-5 items-center justify-center'>
                        ...
                        <div>
                            <div className='flex gap-2 items-center p-2 rounded shadow'>
                                ...
                                <input type="date" onChange={(e) => setFilterDate(e.target.value)} className='text-sm px-1 py-1 border outline-none flex items-center gap-1' />
                                <button onClick={onFilterByDate} className='text-sm bg-[#ff00a2] text-white px-2 py-1 rounded'>Apply</button>
                            </div>
                        </div>
                    </div>
